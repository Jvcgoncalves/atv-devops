...
import type { RoomRow, SensorRow, TelemetryInsert, TelemetryResult, ThresholdRow } from "./types.ts";
...
export function inserirLeitura({ id_sensor, valor, qualidade = null }: TelemetryInsert): TelemetryResult {
  const sensor = db.prepare("SELECT * FROM SENSOR WHERE id_sensor = ?").get(id_sensor) as SensorRow | undefined;
...
function avaliarLimites(sensor: SensorRow, valor: number, data_hora: string): void {
  const lim = db
    .prepare("SELECT * FROM LIMITE_ALERTA WHERE id_sala = ? AND tipo = ?")
    .get(sensor.id_sala, sensor.tipo) as ThresholdRow | undefined;
  if (!lim) return;
  ...
  const sala = db.prepare("SELECT * FROM SALA WHERE id_sala = ?").get(sensor.id_sala) as RoomRow | undefined;
  const nivel = "critico";
...
async function enviarNtfy(nivel: string, mensagem: string): Promise<void> {
  ...
  try {
    ...
    console.error("Falha ao enviar ntfy:", e instanceof Error ? e.message : String(e));
  }
}
