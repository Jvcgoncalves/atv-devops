...
import { startMqtt } from "./mqttBridge.ts";
import type { CrudColumn, TelemetryInsert } from "./types.ts";
...
function colunas(tabela: string): string[] {
  return (db.prepare(`PRAGMA table_info(${tabela})`).all() as CrudColumn[]).map((c) => c.name);
}

function crud(rota: string, tabela: string, pk: string): void {
...
app.post("/api/telemetria", (req: Request<Record<string, string>, unknown, TelemetryInsert>, res: Response) => {
