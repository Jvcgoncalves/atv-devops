...
import { statusColor, STATUS } from "../theme.ts";
import StatusBadge from "./StatusBadge.tsx";
import type { MetricProps, RoomCardProps } from "../types/ui.ts";

function Metric({ label, icon, value, unit, status, digits = 1 }: MetricProps) {
  return (
...
export default function RoomCard({ room, climatizador }: RoomCardProps) {
        ...
        {!auto && (
          <div className="range">
            <input
              ...
              onMouseUp={(e) => setVav(room.id, Number(e.currentTarget.value))}
              onTouchEnd={(e) => setVav(room.id, Number(e.currentTarget.value))}
            />
