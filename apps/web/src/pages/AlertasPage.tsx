...
import type { Alert, AlertLevel, NtfyLogEntry } from "@hvac/contracts";
import type { ConfigContextValue } from "../types/ui.ts";

const ICON: Record<AlertLevel, string> = { critico: "🚨", atencao: "⚠️", info: "ℹ️" };
const FILTERS = ["ativos", "criticos", "todos"] as const;

function AlertItem({ alert, onAck }: { alert: Alert; onAck: ConfigContextValue["acknowledgeAlert"] }) {
...
export default function AlertasPage() {
  ...
  const [ntfyLog, setNtfyLog] = useState<NtfyLogEntry[]>([]);
  const [filter, setFilter] = useState<"ativos" | "criticos" | "todos">("ativos");
