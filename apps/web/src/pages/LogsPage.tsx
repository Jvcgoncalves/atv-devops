...
import type { AuditCategory, AuditEvent, Identification } from "@hvac/contracts";

const CATEGORIAS: Record<AuditCategory, { txt: string; cor: string }> = {
...
function CatBadge({ categoria }: { categoria: AuditCategory }) {
...
const esc = (c: unknown): string => `"${String(c ?? "").replace(/"/g, '""')}"`;
...
function exportarCSV(rows: AuditEvent[], ident: Partial<Identification>) {
