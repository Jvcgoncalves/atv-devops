...
import type { NtfyConfig, RoomThresholds, Room } from "@hvac/contracts";

function Field({ label, value, onChange, step = "1", min }: { label: string; value: number; onChange(value: string): void; step?: string; min?: string }) {
  return (
    <div className="field">
      <label>{label}</label>
...
export default function ParametrosPage() {
  ...
  const [form, setForm] = useState<RoomThresholds | null>(null);
  ...
  const upd = (group: keyof RoomThresholds, key: string, val: string) =>
    setForm((f) => f ? { ...f, [group]: { ...f[group], [key]: Number(val) } } : f);

  const handleSave = async () => {
    if (!salaId || !form) return;
        ...
        <div className="card">
          ...
          <div className="form-row" style={{ gridTemplateColumns: "1fr 1fr" }}>
            <div className="field">
              ...
              <select value={ntfy.minLevel} onChange={(e) => setNtfy({ ...ntfy, minLevel: e.target.value as NtfyConfig["minLevel"] })}>
